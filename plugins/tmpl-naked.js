...
let processTmpl = (tmpl, shortFrom) => {
    let extra = {
        shortHTMLFile: shortFrom
    };
    let shared = {};
    tmpl = configs.compileTmplStart(tmpl, shared, extra);
    let info = tmplSource.store(fileContent);
    ...
    tmpl = tmplSource.recover(tmpl, info.recoverReg, info.store);
    tmpl = configs.compileTmplEnd(tmpl, shared, extra);
    return tmpl;
};
let processMx = (content, shortFrom) => {
    content = content.replace(templateReg, (match, lang, body) => {
        ...
        return '<template' + (lang || '') + '>' + processTmpl(body, shortFrom, tLang) + '</template>';
    });
    return content;
};
module.exports = {
